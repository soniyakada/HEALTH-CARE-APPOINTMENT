...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!contactNumber.trim())
      return setContactError("Please enter your contact number.");
    if (!indianContactRegex.test(contactNumber.trim()))
      return setContactError("Please enter a valid mobile number.");
    if (!password.trim()) return setPassError("Password cannot be empty.");
    if (!address.trim()) return setAddError("Please enter address.");

    if (role === "doctor") {
      if (!specialization.trim())
        return setSpecialError("Specialization is required.");
      if (!availability.trim())
        return setAvailabilityError("Availability is required.");
        ...
        <div className="container">
          <form onSubmit={handleSubmit} className="signup-form">
            ...
            <div>
              ...
              />
              {contactError && (
                <span className="text-xs text-red-600">{contactError}</span>
              )}
            </div>
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    onFocus={() => setSpecialError("")}
                  />
                  {specialError && (
                    <span className="text-xs text-red-600">{specialError}</span>
                  )}
                </div>
                ...
                <div>
                  ...
                  />
                  {feeError && (
                    <span className="text-xs text-red-600">{feeError}</span>
                  )}
                </div>
              </>
            )}
